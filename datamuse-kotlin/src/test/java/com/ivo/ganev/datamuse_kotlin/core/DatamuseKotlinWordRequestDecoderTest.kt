...
class DatamuseKotlinWordRequestDecoderTest {
    private val absolutePath = "src\\test\\resources\\"
    ...
    private lateinit var wordResponseDecoder: WordResponseDecoder

    @Before
    fun setUp() {
        wordResponseDecoder = WordResponseDecoder()
    ...
    fun wordDecoderWithMalformedJsonBodyThrowsException() {
        try {
            wordResponseDecoder.decode("malformed url")
            fail("Didn't throw")
        }
    ...
    fun wordDecoderDecodesCorrectlyTagRequests() {
        // Arrange
        val file = File("$absolutePath$TAGS").readText()

        // Act - make word decoder to decode the provided JSON file
        val wordResponseSet = wordResponseDecoder.decode(file)
    ...
    fun wordDecoderDecodesCorrectlyDefinitionsRequests() {
        // Arrange
        val file = File("$absolutePath$DEFINITIONS").readText()
